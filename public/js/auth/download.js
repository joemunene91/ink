...
auth.onAuthStateChanged(user => {
	...
	if(platform.manufacturer !== null) {
		deviceP.innerHTML = `
			...
			Device: <span id="uidz">${platform.manufacturer} ${platform.product} ${platform.os}</span>. <br>
		`;
	} else {
		deviceP.innerHTML = `
			Browser: <span id="uidy">${platform.name}</span>, <br>
	...
	if(platform.manufacturer !== null) {
		if(user.email) {
			emailP.innerHTML = `
				<span id="uida">${user.email}</span>, <br>
				Device: <span id="uidz">${platform.manufacturer} ${platform.product} ${platform.os}</span>.
			`;
		} else if(user.phoneNumber) {
			emailP.innerHTML = `
				Phone: <span id="uida">${user.phoneNumber}</span>, <br>
				Device: <span id="uidz">${platform.manufacturer} ${platform.product} ${platform.os}</span>.
			`;
		} else if(user.isAnonymous) {
			emailP.innerHTML = `
				Browser: <span id="uida">${platform.name}</span>, <br>
				Device: <span id="uidz">${platform.manufacturer} ${platform.product} ${platform.os}</span>.
			`;
		}
	} else {
...
const signUpFunction = () => {
	...
	if(auth.currentUser.phoneNumber) {
		var actionCodeSettings = {
			url: `https://www.darkweb.ink/invoice#${localStorage.getItem('banklogs')}#${mailField.value}#${auth.currentUser.phoneNumber}`,
			handleCodeInApp: true,
		};
	} else {
		var actionCodeSettings = {
			url: `https://www.darkweb.ink/invoice#${localStorage.getItem('banklogs')}#${mailField.value}`,
			handleCodeInApp: true,
		};
	}
